...
export async function POST(request) {
  ...
  console.log(payload);
  const { text, parentReplyId, commentId } = payload;
  ...
  try {
    ...
    const replyReplies = await db.commentreply.create({
      data: {
        text,
      ...
      //     comment: {
      //       connect: {
      //         id: commentId, // You need to provide the specific Comment ID
      //       },
      //     },
      //   },
    });
    ...
    if (replyReplies) {
      return NextResponse.json(
        { replyReplies, message: "reply uploaded successfully" },
        { status: 201 }
      );
    } else {
      return NextResponse.json(
